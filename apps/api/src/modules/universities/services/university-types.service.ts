...
export class UniversityTypeService {
  static async listAll(query: ListUniversityTypesQuery) {
    return UniversityTypeRepository.listAll({
      isActive: query.is_active,
    });
  }

  static async getById(id: string) {
    const type = await UniversityTypeRepository.findById(id);
    if (!type) throw new NotFoundError("University type not found");
    return type;
  }

  static async create(data: CreateUniversityTypeInput) {
    const existingByName = await UniversityTypeRepository.findByName(data.name);
    if (existingByName)
      throw new ConflictError("University type name already exists");

    const existingBySlug = await UniversityTypeRepository.findBySlug(data.slug);
    if (existingBySlug)
      throw new ConflictError("University type slug already exists");

    return UniversityTypeRepository.create({
      name: data.name,
      slug: data.slug,
  ...
  static async update(id: string, data: UpdateUniversityTypeInput) {
    ...
    if (data.name && data.name !== existing.name) {
      const existingByName = await UniversityTypeRepository.findByName(
        data.name,
      );
      if (existingByName) {
        throw new ConflictError("University type name already exists");
    ...
    if (data.slug && data.slug !== existing.slug) {
      const existingBySlug = await UniversityTypeRepository.findBySlug(
        data.slug,
      );
      if (existingBySlug) {
        throw new ConflictError("University type slug already exists");
    ...
    return UniversityTypeRepository.updateById(id, {
