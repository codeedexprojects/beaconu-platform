...
import { platformRolesSchemas } from './platform-roles.schema';
...
export class PlatformRolesController {
  static async listPermissions(_req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res.status(200).json(ApiResponse.success('Platform permissions fetched successfully', permissions));
    } catch (error) {
      next(error);
    }
  }
  ...
  static async listRoles(_req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res.status(200).json(ApiResponse.success('Platform roles fetched successfully', roles));
    } catch (error) {
      next(error);
    }
  }
  ...
  static async updateRolePermissions(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await PlatformRolesService.updateRolePermissions(roleId, data);

      return res.status(200).json(
        ApiResponse.success('Platform role permissions updated successfully', result)
      );
    } catch (error) {
      next(error);
    }
