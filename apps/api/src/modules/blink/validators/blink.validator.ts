...
export const registerAssociateEmployeeSchema = z
  .object({
    ...
    email: commonSchemas.email,
    phone_number: z.string().optional(),
    associate_parent_id: z.string().trim().refine(
      (value) => !value.includes('{{') && !value.includes('}}') && uuidRegex.test(value),
...
export type RegisterAssociateEmployeeInput = z.infer<typeof registerAssociateEmployeeSchema>;
