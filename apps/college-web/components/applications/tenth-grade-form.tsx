...
const tenthGradeSchema = z.object({
...
type TenthGradeFormInput = z.infer<typeof tenthGradeSchema>;
...
export function TenthGradeForm({ applicationId }: TenthGradeFormProps) {
  const { data: existingSection, isLoading } = useFormDetails(
    applicationId,
    "qualification_details",
    true,
  );
  const existing = existingSection?.tenth_grade;
  const { mutate: save, isPending } = useUpdateTenthGradeDetails(applicationId);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [isUploading, setIsUploading] = useState(false);

  const form = useForm<TenthGradeFormInput>({
    resolver: zodResolver(tenthGradeSchema),
    defaultValues: {
      academic_year: "",
      admission_year: "",
  ...
  async function handleMarksheetChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      form.setValue("marksheet_url", uploaded.url, { shouldValidate: true });
    } catch (error) {
  ...
  function onSubmit(data: TenthGradeFormInput) {
    save(
      ...
      {
        onSuccess: () => {
          toast.success("10th grade details saved");
        },
      },
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        >
          <Input placeholder="2022-23" {...form.register("academic_year")} />
        </Field>
        <Field
